...
from backend.domains.site_scanning.entities import CrawlPolicy
...
from backend.domains.site_scanning.responses import (
    browser_link_response,
    site_auth_profile_response,
    site_auth_readiness_response,
    site_scan_adapter_response,
    site_scan_response,
)
...
def list_adapters(
    ...
    return [
        site_scan_adapter_response(record, service.runner_manifest_for(record.site_key))
...
def update_adapter(
    ...
    return site_scan_adapter_response(record, service.runner_manifest_for(record.site_key))
...
def list_site_auth_profiles(
    ...
    return [
        site_auth_profile_response(declaration, record)
...
def upsert_site_auth_profile(
    ...
    return site_auth_profile_response(declaration, record)
...
def start_site_auth_browser_link(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return browser_link_response(declaration, started, site_auth_profile_response(declaration, record))
...
def capture_site_auth_browser_link(
    ...
    return browser_link_response(
        declaration,
        status,
        site_auth_profile_response(declaration, record) if record is not None else None,
    )
...
def test_site_auth_profile(
    ...
    return site_auth_readiness_response(declaration, record, readiness)
...
def create_scan(
    ...
    return site_scan_response(result, scan_run_id=scan_run.id)
